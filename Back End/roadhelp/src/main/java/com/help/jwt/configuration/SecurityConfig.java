...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf(csrf -> csrf.disable()).sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .authorizeHttpRequests(auth -> auth.requestMatchers("/auth/user/register", "/auth/user/login","/auth/admin/register","/auth/user/login","/auth/user/email/otp","/auth/user/otp/verify","/auth/admin/email/otp","/auth/admin/otp/verify").permitAll().anyRequest().authenticated())
